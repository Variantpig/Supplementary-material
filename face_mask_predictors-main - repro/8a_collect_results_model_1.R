...
# =========================================================
# 2. Helper function
#    Read one model result and return one-row summary
# =========================================================
...
collect_one_model_result <- function(model_number, model_type) {
  ...
  # -------------------------------------------------------
  # Case 1: fold-level results exist in .rds
  # -------------------------------------------------------
  if (file.exists(rds_path)) {
    M <- readRDS(rds_path)
    
    # Standardise column names across different model scripts
  ...
  # -------------------------------------------------------
  # Case 2: only summary csv exists
  # -------------------------------------------------------
  if (file.exists(csv_path)) {
    M <- read_csv(csv_path, show_col_types = FALSE)
    
    # Compatible with your current R summary files
    model_df <- tibble(
      model_number = model_number,
      model_type = model_type,
